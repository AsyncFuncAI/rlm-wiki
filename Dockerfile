...
# Optional Unikraft / remote worker image target (NOT the default final stage):
...
# Default final stage must remain the web service. Leaving `worker` last made
# plain `docker build` / Railway images boot as workers and break /api serve.
FROM base AS web
CMD ["bun", "run", "./bin/rlm-wiki.ts", "serve"]
