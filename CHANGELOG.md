...
- Prepared rlm-wiki 0.0.33 with an Orca-style Terminal projects rail, improved TUI fidelity, and project-aware local-agent workspaces.
...
- Released rlm-wiki 0.0.30 with desktop Ask reliability fixes for Local CLI agents that depend on login-shell proxy and certificate settings.
...
- Released rlm-wiki 0.0.28 with server-side PostHog error capture for the desktop local server and API flows.
...
- Released rlm-wiki 0.0.27 with a local-first Tasks board that turns Ask findings into backlog cards, epics, and sub-tasks.
...
- Released rlm-wiki 0.0.26 with Knowledge Bases that turn Ask sessions into source-backed study cards.
...
- Released rlm-wiki 0.0.25 with a floating outline peek for multi-turn Ask sessions so long answers stay navigable without leaving the current turn.
...
- Released rlm-wiki 0.0.24 with warmer 90s memory-style desktop entry surfaces for Projects, Wiki, Ask, Docs, and the launchpad.
...
- Released rlm-wiki 0.0.23 with packaged macOS terminal-agent launch fixes for local CLI binaries detected outside the app's narrower GUI PATH.
...
- Released rlm-wiki 0.0.22 with desktop completion attention for long-running wiki and Ask work: native alerts, optional sound, dock badge counts, and deep links back into the finished item.
...
- Released rlm-wiki 0.0.17 with a more capable Docs Chat overlay: answers now stay on the docs page, support follow-up or new questions, and prioritize the current documentation MDX before falling back to repository inspection.
...
- Strengthened the public GitHub release runbook so `AsyncFuncAI/rlm-wiki` main and release tags stay limited to `README.md` and `skills/`.

## 2026-06-01

- Released rlm-wiki 0.0.16 with first-class documentation support: the desktop app can now generate Docs from repositories, keep docs separated from regular wikis, and render documentation pages with a docs-style reader, navigation, page rail, and supported MDX-style components.
...
- Improved the documentation generator contract with docs-specific structure planning, frontmatter validation, planned-route link checks, Docs MDX component guidance, and safeguards against visible source-citation clutter in docs bodies.
...
- Added bundled rlm-wiki agent skills and local-first source/runtime documentation for BYOK/BYOC operation.
- Updated desktop updater handling and release verification coverage for signed macOS artifacts and Tauri updater metadata.

## 2026-05-27

- Released rlm-wiki 0.0.15 with first-class local folder support for wiki generation when a selected folder does not contain a `.git` directory.
